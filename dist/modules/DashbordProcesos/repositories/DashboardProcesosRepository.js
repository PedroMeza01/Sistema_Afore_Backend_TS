...
exports.DashboardProcesosRepository = {
    listPaginated: async (input) => {
      ...
      `
        ...
        const countRows = await db_1.dbLocal.query(`
      WITH base AS (
        SELECT
          p.id_proceso,
          p.id_cliente,
      ...
      `, {
        ...
        // ITEMS paginados
        const items = await db_1.dbLocal.query(`
      WITH base AS (
        SELECT
          p.id_proceso,
          p.id_cliente,
    ...
    getKPIs: async (input) => {
        ...
        const kpiSimple = await Proceso_1.default.findOne({
            where: { id_organizacion },
            attributes: [
                [sequelize_1.Sequelize.literal(`SUM(CASE WHEN estatus_proceso = 'ACTIVO' THEN 1 ELSE 0 END)`), 'activos'],
                [sequelize_1.Sequelize.literal(`SUM(CASE WHEN estatus_proceso = 'BLOQUEADO' THEN 1 ELSE 0 END)`), 'bloqueados'],
                [sequelize_1.Sequelize.literal(`SUM(CASE WHEN estatus_proceso = 'CANCELADO' THEN 1 ELSE 0 END)`), 'cancelados'],
                [sequelize_1.Sequelize.literal(`SUM(CASE WHEN tramite_solicitado = true THEN 1 ELSE 0 END)`), 'tramite_solicitado'],
                [
                    sequelize_1.Sequelize.literal(`SUM(CASE WHEN tramite_solicitado = true AND (resultado_tramite IS NULL OR resultado_tramite = '') THEN 1 ELSE 0 END)`),
                    'tramite_sin_resultado'
                ],
                [sequelize_1.Sequelize.literal(`SUM(CASE WHEN listo_para_cobro = true THEN 1 ELSE 0 END)`), 'listos_para_cobro'],
                [
                    sequelize_1.Sequelize.literal(`SUM(CASE WHEN listo_para_cobro = true AND (fecha_cobro IS NULL OR fecha_cobro > '${today}') THEN 1 ELSE 0 END)`),
                    'pendientes_por_cobrar'
                ],
                [sequelize_1.Sequelize.literal(`COALESCE(SUM(COALESCE(comision_asesora,0)::numeric),0)`), 'comision_total'],
                [sequelize_1.Sequelize.literal(`COALESCE(SUM(COALESCE(bono_asesora,0)::numeric),0)`), 'bono_total'],
                [
                    sequelize_1.Sequelize.literal(`SUM(CASE WHEN cita_afore IS NOT NULL AND cita_afore >= '${today}' AND cita_afore <= '${todayPlus30}' THEN 1 ELSE 0 END)`),
                    'citas_proximas_7'
                ],
                [
                    sequelize_1.Sequelize.literal(`SUM(CASE WHEN cita_afore IS NOT NULL AND cita_afore < '${today}' THEN 1 ELSE 0 END)`),
                    'citas_vencidas'
                ],
                [
                    sequelize_1.Sequelize.literal(`SUM(CASE WHEN fecha_46_dias IS NOT NULL AND fecha_46_dias >= '${today}' AND fecha_46_dias <= '${todayPlus30}' THEN 1 ELSE 0 END)`),
                    'dias46_proximos_7'
                ],
                [
                    sequelize_1.Sequelize.literal(`SUM(CASE WHEN fecha_46_dias IS NOT NULL AND fecha_46_dias < '${today}' THEN 1 ELSE 0 END)`),
                    'dias46_vencidos'
                ],
                [
                    sequelize_1.Sequelize.literal(`SUM(CASE WHEN tramite_solicitado = true AND (expediente_actualizado = false OR app_vinculada = false) THEN 1 ELSE 0 END)`),
        ...
        const docsStats = await db_1.dbLocal.query(`
      WITH per_proceso AS (
        SELECT
          p.id_proceso,
...
//# sourceMappingURL=DashboardProcesosRepository.js.map